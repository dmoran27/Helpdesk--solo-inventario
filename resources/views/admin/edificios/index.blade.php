                    ...
                    @foreach($edificios as $key => $edificio )
                        <tr data-entry-id="{{ $edificio->id }}">
                            ...
                            <td>
                                ...
                                @endcan
                                @can('edificio_delete')
                                    <form action="{{ route('admin.edificios.destroy', $edificio->id) }}" method="POST" class="w-100 d-inline-block formularioEliminar" id="formularioEliminar{{$edificio->id}}" 
                                      ">
                                        <input type="hidden" name="_method" value="DELETE">
                                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                        <button id="eliminar" class="btn w-100 btn-xs btn-danger eliminar" value="{{$edificio->id}}">Eliminar</button>
                                    </form>
                                @endcan
                            </td>
